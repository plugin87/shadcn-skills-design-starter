...
2. Check if it already exists: `.claude/skills/_ux-ui-shared/components/atoms.md`, `molecules.md`, `organisms.md`, `templates.md`, `navigation.md`, `feedback.md`, `forms-advanced.md`, `overlays.md`. Match the existing spec format.
3. Pull the ARIA pattern from `.claude/skills/_ux-ui-shared/accessibility/aria-patterns.md` and contrast/target rules from `.claude/skills/_ux-ui-shared/accessibility/wcag-checklist.md`.
4. Map every value to tokens (`.claude/skills/_ux-ui-shared/tokens/*.json`) — sizes via `sizing.json`, states via `states.json`.
5. Apply visual judgment from `.claude/skills/_ux-ui-shared/taste/design-taste.md` (states, focus, no slop).
6. Optional fast start: `python3 .claude/skills/_ux-ui-shared/scripts/scaffold_component.py "<Name>"` to emit a stub, then fill it in.

## Output
Spec with: anatomy diagram, variants table, sizes table, all 8 applicable states, token mapping, accessibility (role/keyboard/SR), and a note to render via `.claude/skills/_ux-ui-shared/frameworks/adapter-protocol.md`.
...
- `node .claude/skills/_ux-ui-shared/scripts/verify_states.mjs <harness> [--dark]` — contrast of every element in default/hover/focus
- `node .claude/skills/_ux-ui-shared/scripts/axe_audit.mjs <harness> [--dark]` — ARIA/role/name/label correctness
- `node .claude/skills/_ux-ui-shared/scripts/measure_render.mjs <harness> [--dark]` — every text element AA
- overlays/modals also: `node .claude/skills/_ux-ui-shared/scripts/verify_focustrap.mjs <harness> --open=<trigger>`
...
Build mobile-first; a fixed-px width that can't shrink is a bug. Run `node .claude/skills/_ux-ui-shared/scripts/verify_responsive.mjs <file|dir>` — it loads each harness at 280/320/414px and fails on any horizontal overflow. The four recurring causes and their fixes:

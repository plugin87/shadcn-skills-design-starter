...
> **In this project, the DTCG `.claude/skills/_ux-ui-shared/tokens/*.json` are upstream; Figma is a downstream sync target** (no longer the sole source). Pull from Figma with `scripts/figma/figma-pull.mjs`; push tokens→Variables to mirror code. For building UI, defer to `shadcn-ui-design`.
...
## Steps
1. Read `.claude/skills/_ux-ui-shared/workflows/figma-integration.md` (token↔Variable mapping, sync directions, MCP usage, parity).
2. Map the 3-tier hierarchy → three Figma collections (Primitives → Semantic → Component) with aliasing; dark/brand/density → Figma **Modes** (`.claude/skills/_ux-ui-shared/tokens/theming.json`).
...
- Every Figma Variable resolves to a token in `.claude/skills/_ux-ui-shared/tokens/*.json` (no orphan hex in designs).
...
- After any token import: `python3 .claude/skills/_ux-ui-shared/scripts/validate_tokens.py` passes.
